...
#define MINIMUM_ITEMS 2
#define FOOD_NAME_LENGTH 129
...
int main(void)
{
    ...
    int iteration;
    for (iteration = MINIMUM_ITEMS; iteration < LUNCH_ITEMS; iteration++)
    {
        ...
        scanf("%128s %i %i", foodName, &lunch[iteration].weight, &lunch[iteration].calories);
        
        // Check if malloc is successful
        if ((lunch[iteration].name = (char *)malloc(Elements(foodName))) == NULL)
        {
            fputs("Out of memory\n", stderr);
            exit(EXIT_FAILURE);
        }
        // Copy user input string to food name using memcpy
        memcpy(lunch[iteration].name, foodName, strlen(foodName) + 1);
    }
